...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <div class="card shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center">
      <h4 class="mb-0"><i class="bx bx-edit me-2 text-primary"></i>Edit Note</h4>
      <a href="{{ route('notes.home') }}" class="btn btn-sm btn-outline-secondary">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-6">
            ...
            <select name="class_id" id="class_id" class="form-select @error('class_id') is-invalid @enderror" required>
              ...
              @foreach($classes as $class)
                <option value="{{ $class->id }}" {{ old('class_id', $note->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
              @endforeach
            </select>
          ...
          <div class="col-md-6">
            ...
            <select name="subject_id" id="subject_id" class="form-select @error('subject_id') is-invalid @enderror" required>
              <option value="{{ $note->subject_id }}" selected>{{ $note->subject->name ?? 'Current subject' }}</option>
            </select>
            @error('subject_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
          </div>
        </div>

        <div class="form-text mb-3">To replace the uploaded file, delete this note and add a new one — editing only updates the title, class, and subject.</div>

        <div class="d-flex justify-content-end gap-2">
          <a href="{{ route('notes.home') }}" class="btn btn-outline-secondary">Cancel</a>
