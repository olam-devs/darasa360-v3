...
<div class="row">
  <div class="col-12">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('system_admin.dashboard') }}">Dashboard</a></li>
        <li class="breadcrumb-item"><a href="{{ route('system_admin.my_schools.index') }}">My Schools</a></li>
        <li class="breadcrumb-item"><a href="{{ route('system_admin.my_schools.view', $school->id) }}">{{ $school->name }}</a></li>
...
<div class="row">
  ...
  <div class="col-md-8">
    <div class="card">
      ...
      <div class="card-body p-0">
        @if($staff->isEmpty())
          <p class="text-muted p-3 mb-0">No staff yet.</p>
        @else
          <div class="table-responsive">
            <table class="table table-hover mb-0">
              <thead class="table-light">
                <tr><th>#</th><th>Username</th><th>Reg No</th><th>Role</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <form method="POST" action="{{ route('system_admin.my_schools.staff.toggle', [$school->id, $member->id]) }}" class="d-inline">
                      @csrf
                      <button class="btn btn-sm btn-{{ $member->status === 'active' ? 'warning' : 'success' }}">{{ $member->status === 'active' ? 'Disable' : 'Enable' }}</button>
                    </form>
                    <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#resetPwdModal{{ $member->id }}">Reset Pwd</button>
                    <form method="POST" action="{{ route('system_admin.my_schools.staff.delete', [$school->id, $member->id]) }}" class="d-inline" onsubmit="return confirm('Delete {{ $member->username }}?')">
                      @csrf @method('DELETE')
                ...
                <div class="modal fade" id="resetPwdModal{{ $member->id }}" tabindex="-1">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <form method="POST" action="{{ route('system_admin.my_schools.staff.reset_password', [$school->id, $member->id]) }}">
                        @csrf
  ...
  <div class="col-md-4">
    <div class="card">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('system_admin.my_schools.staff.create', $school->id) }}">
          @csrf
          ...
          <div class="mb-3"><label class="form-label">Phone</label><input type="text" name="phone_number" class="form-control"></div>
          <div class="mb-3">
